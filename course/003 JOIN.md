...
Множество табличек хранится в базе данных (Postgres/MySQL/Oracle - как говорится, и другие названия БД (хотя на самом деле правильнее СУБД - система управления базами данных, ибо в одной системе может быть даже несколько баз данных) могут вам встретится в будущем, но самое популярное на данный момент - это Postgres, хотя на самом деле это зачастую GreenPlum, но о нем стоит читать только если вы хотите заснуть или вам некуда девать свое свободное время) 
...
Обычно организация табличек в слои сделана с целью удобства, ну это как сортировать свои носки, чтобы они не были разбросаны по комнате 
...
Давайте уже поговорим про тот тип JOIN'а, с которого обычно начинают изучение всех JOIN'ов 
...
На работе, на собеседованиях, везде при упоминании JOIN'ов вы будете вспоминать их - дубли
...
Представьте, что вы хотите сказать SQL че-то вроде такой команды: "те пары, которые по условию ON совпадают - заджоинь, а оставшиеся (с TRUE условием в O)N строчки из таблицы A и таблицы B - тоже докинь в результат, но, соответственно, с NULL'ами"
...
На самом деле в таком случае будет CROSS JOIN вне зависимости от того какой тип джоина вы написали из-за того, что 9 всегда больше нуля по законам математики 

И да, как вы поняли, так действительно можно джоинить

Ребят, пожалуйста, запомните, распечатайте и повторяйте каждый раз перед сном:

В условие ON можно запихнуть любое выражение, которое однозначно принимает либо FALSE либо TRUE 

Ну и курите следующий пост, если хотите подробностей -> https://t.me/it_pancakes/158

## [8] Минимум и максимум 

Приготовьтесь, сейчас будет то, ради чего мы изучали JOIN'ы - для того, что отвечать на вопросы по ним на собеседованиях

Зачастую, вы даже не дойдете до технического собеседования, как уже на HR собесе вас спросят самый популярный и жесткий вопрос про них 

Такой вопрос строится +- по следующему шаблону

Вам дают в качестве условия 2 таблицы (пусть будут таблицы А и Б) и говорят кол-во строчек в них

И затем выдают следующее: "а какое количество строчек максимальное и минимальное может быть при INNER/LEFT/RIGHT/FULL/CROSS JOIN'e эти таблиц?"

И прикол тут в том, что многие думают, что JOIN находит первую попавшуюся строчку и JOIN'ит ее, даже если есть несколько подходящих строчек. Но мы с вами-то знаем уже, что любой джоин - это по сути симуляция CROSS JOIN'a с последующей фильтрацией строчек и/или конкретных значений в столбцах, так что JOIN может дублировать исходную строчку, если нашел к ней сразу несколько совпадений.

И это действительно важно, ибо еще на своей первой работе я своими глазами увидел, как может быть страшно незнание этой механики, когда после джоина справочника к заказам получается увеличение количества строчек в 2 раза на пустом месте и потом долго и муторно разбираемся, а что не так с формулой - а с формулой все хорошо, просто JOIN задублил данные 
...
И давайте разберем более реалистичный пример на INNER JOIN из собеседования

Пусть в А - 10 строчек, в Б - 5 строчек

Тогда минимум = 0 (когда не будет совпадений)

Максимум = 50 (когда в А и Б таблицах лежит одно и то же число, например)

И тут вы можете воскликнуть: "да это просто же запомнить, че такого, просто при INNER'e запоминаем, что минимум = 0, а максимум = нужно перемножить кол-во строк"

Не советую изучать джоины путем такой зубрежки

Ибо я немного меняю условие задачи и вы уже плывете:

Пусть в А - 10 строчек, в Б - 5 строчек, где в первых двух строчках числа 1 и 2, а остальных - может быть что угодно
